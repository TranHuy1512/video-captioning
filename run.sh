...
# ─── Fresh adapter training (new features, skip old adapter weights) ───
torchrun --nproc_per_node=1 --standalone main_task_caption.py \
--do_train --stage_two --task_type caption --datatype msrvtt \
--num_thread_reader=4 --epochs=5 --batch_size=128 --n_display=50 \
--train_csv data/msrvtt/MSRVTT_train.9k.csv --val_csv data/msrvtt/MSRVTT_JSFUSION_test.csv \
--data_path data/msrvtt/MSRVTT_data.json --features_path feature_path \
--output_dir ckpts/ckpt_msrvtt_caption --bert_model bert-base-uncased \
--do_lower_case --lr 3e-5 --max_words 48 --max_frames 1352 --batch_size_val 128 \
--init_model /workspace/project/video-captioning/ckpts/ckpt_msrvtt_caption_ver_1/pytorch_model.bin.best.0 \
--gradient_accumulation_steps=2 --video_dim 3584 --direct_qformer_input \
--qformer_adapter_type temporal --adapter_temporal_kernel 5 \
--adapter_pool_size 256 \
--skip_init_adapter \
--qformer_vision_width 1408 \
--qformer_checkpoint Salesforce/blip2-opt-6.7b-coco \
--lora --lr_qformer 1e-5 --lr_lora 5e-6 --lora_r=32 \
--lora_alpha=64 --scst --max_txt_len 20 \

# ─── Reuse old model trained on SAME features (load adapter weights) ───
# torchrun --nproc_per_node=1 --standalone main_task_caption.py \
# --do_train --stage_two --task_type caption --datatype msrvtt \
# --num_thread_reader=4 --epochs=5 --batch_size=128 --n_display=50 \
# --train_csv data/msrvtt/MSRVTT_train.9k.csv --val_csv data/msrvtt/MSRVTT_JSFUSION_test.csv \
...
# --output_dir ckpts/ckpt_msrvtt_caption --bert_model bert-base-uncased \
# --do_lower_case --lr 3e-5 --max_words 48 --max_frames 1352 --batch_size_val 128 \
# --init_model /path/to/old_model_same_features/pytorch_model.bin.best.0 \
# --gradient_accumulation_steps=2 --video_dim 3584 --direct_qformer_input \
# --qformer_adapter_type temporal --adapter_temporal_kernel 5 \
# --adapter_pool_size 256 \
# --no_skip_init_adapter \
# --qformer_vision_width 1408 \
# --qformer_checkpoint Salesforce/blip2-opt-6.7b-coco \
# --lora --lr_qformer 1e-5 --lr_lora 5e-6 --lora_r=32 \
# --lora_alpha=64 --scst --max_txt_len 20 \
