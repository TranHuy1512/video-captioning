...
def get_args(description='UniVL on Caption Task'):
    ...
    parser.add_argument('--lr_lora', dest='lr_lora', type=float, default=1e-5,
                        help='Learning rate for Llama LoRA/decoder parameters.')
    ...
    parser.add_argument('--llama_model', type=str, default='meta-llama/Llama-3.2-3B-Instruct',
                        help="Llama decoder model name.")
    parser.add_argument('--hf_token', type=str, default=None,
                        help="Hugging Face access token for gated models such as Llama.")
    parser.add_argument('--max_txt_len', type=int, default=32, help="Maximum text length for Llama tokenizer.")
    ...
    parser.add_argument('--lora', action='store_true', help="Enable LoRA for Llama.")
...
def main():
    ...
    llama_tokenizer = getattr(model, 'llama_tokenizer', None)
    ...
    test_dataloader, test_length = DATALOADER_DICT[args.datatype]["val"](args, tokenizer, logger, llama_tokenizer=llama_tokenizer)
    if args.local_rank == 0:
    ...
    if args.do_train:
        train_dataloader, train_length, train_sampler = DATALOADER_DICT[args.datatype]["train"](args, tokenizer, llama_tokenizer=llama_tokenizer)
