...
def get_args(description='UniVL on Caption Task'):
    ...
    parser.add_argument('--lr_lora', '--lr_decoder', dest='lr_lora', type=float, default=1e-5,
                        help='Learning rate for Phi LoRA/decoder parameters.')
    ...
    parser.add_argument('--lora', action='store_true', help="Enable LoRA for Phi.")
...
def main():
    ...
    decoder_tokenizer = getattr(model, 'phi_tokenizer', None)
    ...
    test_dataloader, test_length = DATALOADER_DICT[args.datatype]["val"](args, tokenizer, logger, decoder_tokenizer=decoder_tokenizer)
    if args.local_rank == 0:
    ...
    if args.do_train:
        train_dataloader, train_length, train_sampler = DATALOADER_DICT[args.datatype]["train"](args, tokenizer, decoder_tokenizer=decoder_tokenizer)
