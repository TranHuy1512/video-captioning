...
def prep_optimizer(args, model, num_train_optimization_steps, device, n_gpu, local_rank, coef_lr=1.):
    ...
    def is_llama_decoder_param(name):
        return name.startswith(("llama_model.", "llama_proj."))

    def is_bert_param(name):
        return name.startswith("bert.")

    def is_other_param(name):
        return not (is_bert_param(name) or is_qformer_param(name) or is_llama_decoder_param(name))
    ...
    no_decay_llama_decoder_param_tp = [(n, p) for n, p in no_decay_param_tp if is_llama_decoder_param(n)]
    ...
    decay_llama_decoder_param_tp = [(n, p) for n, p in decay_param_tp if is_llama_decoder_param(n)]
    ...
    optimizer_grouped_parameters = [
        ...
        {'params': [p for n, p in no_decay_llama_decoder_param_tp], 'weight_decay': 0.01, 'lr': lr_lora},
        ...
        {'params': [p for n, p in decay_llama_decoder_param_tp], 'weight_decay': 0.0, 'lr': lr_lora},
