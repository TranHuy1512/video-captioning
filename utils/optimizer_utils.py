...
def prep_optimizer(args, model, num_train_optimization_steps, device, n_gpu, local_rank, coef_lr=1.):
    ...
    def is_cross_param(name):
        return name.startswith("cross.")
    ...
    def is_other_param(name):
        return not (is_bert_param(name) or is_cross_param(name) or is_t5_decoder_param(name))

    lr_cross = getattr(args, "lr_cross", args.lr)
    ...
    no_decay_cross_param_tp = [(n, p) for n, p in no_decay_param_tp if is_cross_param(n)]
    ...
    decay_cross_param_tp = [(n, p) for n, p in decay_param_tp if is_cross_param(n)]
    ...
    optimizer_grouped_parameters = [
        ...
        {'params': [p for n, p in no_decay_cross_param_tp], 'weight_decay': 0.01, 'lr': lr_cross},
        ...
        {'params': [p for n, p in decay_cross_param_tp], 'weight_decay': 0.0, 'lr': lr_cross},
