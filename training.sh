# ─── CE training with STC features (SigLIP → STC → 1352×3584) ───
# Set --adapter_pool_size to 256 or 512 to test both.
# Use --max_frames >= 1352 so dataloader doesn't mean-pool.
torchrun --nproc_per_node=1 --standalone main_task_caption.py \
	...
	--lr 5e-6 --max_words 48 --max_frames 1352 --batch_size_val 256 \
	...
	--max_txt_len 32 \
	--direct_qformer_input --qformer_adapter_type temporal --adapter_temporal_kernel 5 \
	...
	# To add SCST later: --scst --scst_alpha 0.7 --scst_num_samples 5 --eval_beam_size 5
