...
def eval_epoch(
    ...
    for batch in test_dataloader:
        ...
        pairs_opt_output_caption_ids = batch
        ...
        with torch.no_grad():
            ...
            loss, visual_output = model(input_ids, segment_ids, input_mask, video, video_mask,
                        ...
                        opt_output_caption_ids=pairs_opt_output_caption_ids)
            if loss is not None:
                if n_gpu > 1:
                    loss = loss.mean()
            ...
            all_result_lists.extend(model.opt_tokenizer.batch_decode(generated_ids, skip_special_tokens=True))
